<?php $__env->startSection('title', 'Search Result - Perch Bottle'); ?>

<?php $__env->startSection('content'); ?>
<div class="page-content search-page-content">
    <!-- Menu bar (mobile) -->
    <div class="breadcrumb-block style-shared">
        <div class="breadcrumb-main bg-linear overflow-hidden">
            <div class="container lg:pt-[134px] pt-24 pb-10 relative">
                <div class="main-content w-full h-full flex flex-col items-center justify-center relative z-[1]">
                    <div class="text-content">
                        <div class="heading2 text-center">Search Result</div>
                        <div class="link flex items-center justify-center gap-1 caption1 mt-3">
                            <a href="<?php echo e(route('home')); ?>">Homepage</a>
                            <i class="ph ph-caret-right text-sm text-secondary2"></i>
                            <div class="text-secondary2 capitalize">Search Result</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="shop-product search-result-block lg:py-20 md:py-14 py-10">
        <div class="container">
            <div class="heading flex flex-col items-center">
                <div class="heading4 text-center">
                    <?php if($query): ?>
                        Found <span class="result-quantity"><?php echo e($products->total()); ?></span> results for "<span class="result"><?php echo e(e($query)); ?></span>"
                    <?php else: ?>
                        <span class="result-quantity">0</span> results - enter a search term
                    <?php endif; ?>
                </div>
                <div class="input-block lg:w-1/2 sm:w-3/5 w-full md:h-[52px] h-[44px] sm:mt-8 mt-5">
                    <form method="GET" action="<?php echo e(route('search')); ?>" class="form-search w-full h-full relative">
                        <input type="text" name="q" placeholder="Search products..." value="<?php echo e(e($query ?? '')); ?>" class="caption1 w-full h-full pl-4 md:pr-[150px] pr-32 rounded-xl border border-line" />
                        <button type="submit" class="button-main absolute top-1 bottom-1 right-1 flex items-center justify-center px-4">Search</button>
                    </form>
                </div>
            </div>

            <div class="list-product-block relative md:pt-10 pt-6">
                <div class="heading6">
                    <?php if($query): ?>
                        Product search: <span class="result"><?php echo e(e($query)); ?></span>
                    <?php else: ?>
                        Search for products
                    <?php endif; ?>
                </div>
                <div class="list-product list-product-result hide-product-sold grid lg:grid-cols-4 sm:grid-cols-3 grid-cols-2 sm:gap-[30px] gap-[20px] mt-5">
                    <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="product-item grid-type">
                            <?php echo $__env->make('partials.product-card', ['product' => $product], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <div class="col-span-full text-center py-12">
                            <?php if($query): ?>
                                <p class="body1 text-secondary">No products found for "<?php echo e(e($query)); ?>". Try different keywords.</p>
                            <?php else: ?>
                                <p class="body1 text-secondary">Enter a search term above to find products.</p>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                </div>
                <?php if($query && $products->hasPages()): ?>
                    <div class="list-pagination w-full flex items-center justify-center gap-4 mt-10">
                        <?php echo e($products->links()); ?>

                    </div>
                <?php endif; ?>
            </div>
        </div>
    </div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/mousamjain/Documents/GitHub/edx/resources/views/search-result.blade.php ENDPATH**/ ?>